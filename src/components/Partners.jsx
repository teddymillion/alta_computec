...
const TIER1 = [
  { name: 'Dell',      tier: 'Platinum Partner', badge: 'tier-platinum', note: "Ethiopia's Only", glow: 'rgba(0,125,184,0.18)',  accentFrom: '#007DB8', accentTo: '#005F8A' },
  { name: 'Cisco',     tier: 'Premier Partner',  badge: 'tier-premier',  note: 'Certified',       glow: 'rgba(27,160,215,0.18)', accentFrom: '#1BA0D7', accentTo: '#0D6E9E' },
  { name: 'Oracle',    tier: 'Gold Partner',     badge: 'tier-gold',     note: 'Certified',       glow: 'rgba(248,0,0,0.15)',   accentFrom: '#F80000', accentTo: '#C00000' },
  { name: 'Kaspersky', tier: 'Platinum Partner', badge: 'tier-platinum', note: 'Certified',       glow: 'rgba(0,109,92,0.18)',  accentFrom: '#006D5C', accentTo: '#004F43' },
];
...
function Tier1Card({ p }) {
  ...
  return (
    <div
      ...
      style={{
        border: isDell ? '1px solid rgba(245,158,11,0.30)' : '1px solid rgba(226,232,240,1)',
        boxShadow: isDell ? '0 1px 3px rgba(0,0,0,0.06), 0 0 0 1px rgba(245,158,11,0.08)' : '0 1px 3px rgba(0,0,0,0.06)',
      }}
      onMouseEnter={(e) => {
        e.currentTarget.style.boxShadow = `0 16px 40px ${p.glow}, 0 4px 8px rgba(0,0,0,0.06)`;
        e.currentTarget.style.background = `radial-gradient(ellipse at center top, ${p.glow.replace(/[\d.]+\)$/, '0.05)')} 0%, #fff 65%)`;
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.boxShadow = isDell ? '0 1px 3px rgba(0,0,0,0.06), 0 0 0 1px rgba(245,158,11,0.08)' : '0 1px 3px rgba(0,0,0,0.06)';
    ...
    >
      ...
      <div className="flex items-center justify-center group-hover:scale-105 transition-transform duration-250" style={{ height: 80 }}>
        <PartnerLogo name={p.name} size={72} />
      </div>
...
function Tier2Item({ p }) {
  return (
    <div className="group flex flex-col items-center gap-3 p-5 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 hover:-translate-y-1 hover:shadow-xl transition-all duration-200 cursor-default">
      <div className="flex items-center justify-center group-hover:scale-110 transition-transform duration-200" style={{ height: 64 }}>
        <PartnerLogo name={p.name} size={64} />
      </div>
      <div className="flex flex-col items-center gap-0.5 text-center">
        <span className="text-[13px] font-semibold text-slate-700 leading-tight">{p.name}</span>
